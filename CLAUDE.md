...
2. **Text is always the opposite of its background.** High contrast, always. Dark text on light bg; light text on dark bg. Never anything in between.
   ...
   - **Named exception(s)** — explicit, never catch-all:
     - **Install-command CTA pattern** (`.install`, `.install-alt`, and equivalents): two carve-outs from the default white-on-black rule, both scoped to install CTAs only:
       1. The `uvx … / pipx install … / pip install …` **command text uses orange `#FF6B35`** on the near-black button bg — recognizable terminal/command-pop aesthetic, the brand pop on the page's primary action.
       2. The **COPY action chip** (`.copy-hint`, `.alt-state`) uses **black bg + cyan `#00ffff` text + cyan border** — chip shape clearly button-like, two brand colors balanced on the same install button (orange command + cyan action), both on near-black.
       Scope is narrow: only inside install CTA buttons. Inline code pills elsewhere (e.g. `.card p code`, `.defer-note code`) still follow the default white-on-black rule. New exceptions must be NAMED here — vibes-based brand-pop arguments don't qualify.
