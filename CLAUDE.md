...
2. **Text is always the opposite of its background.** High contrast, always. Dark text on light bg; light text on dark bg. Never anything in between.
   ...
   - **Named exception(s)** — explicit, never catch-all:
     - **Install-command CTA pattern** (`.install`, `.install-alt`, and equivalents): ONE carve-out only — the `uvx … / pipx install … / pip install …` **command text uses orange `#FF6B35`** on the near-black button bg. This is the recognizable terminal/command-pop aesthetic. Scope is narrow: only the install-command text inside install CTA buttons. The COPY action chip on the SAME button follows the default white-on-black rule (NOT a second brand-color exception — see Action-Subordination principle below). Inline code pills elsewhere (e.g. `.card p code`, `.defer-note code`) also default white-on-black. New exceptions must be NAMED here — vibes-based brand-pop arguments don't qualify.
