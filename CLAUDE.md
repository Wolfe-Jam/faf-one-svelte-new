...
4. **Brand colors only: orange + cyan.**
   ...
   - **Named exception — data-map lines** (wolfejam 2026-09-15, the Pack of Cards map): a chart that has to tell five or more things apart may colour its **data marks only** (lines, ticks, key swatches) from a validated set beyond orange + cyan. Never text, buttons, backgrounds or borders. Current set: A2A Agent Card blue `#2a78d6` (dark `#2068cb`), AI Catalog amber `#eda100` (dark `#c98500`), .fafa **FAFA cyan `#1bb4c4`**, the cyan of the FAFA mark (dark step `#20a7ba`: `#1bb4c4` sits above the dark lightness band on `#0a0a0a`). Family colours: orange belongs to `.faf` and `.fafm`, cyan to `.fafa`, so the map gives .fafa cyan (wolfejam 2026-09-15). AI Catalog left aqua because aqua and cyan fail the normal-vision floor together. MCP Server Card is an ink line (`var(--faf-black)`) and ARD a hollow ink line, so the set stays at three hues. Identity never rests on colour alone: every line is labelled at both ends, with a key and a table view. On cream, aqua and orange sit under 3:1, so those labels and the table are required, not optional. A new colour joins only after the dataviz palette validator passes on both grounds (`#FEFCF8`, `#0a0a0a`) with `--pairs all`. Everything else on a data-map page follows rules 1–11.
